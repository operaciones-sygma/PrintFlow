...
// PRINTFLOW v10.9.0 — Folio Anticipado + Captura Manual (Karla)
...
const db = {
  ...
  // Calcula MAX(invoice_folio) por tipo y devuelve el siguiente número.
  // Más robusto que leer invoice_counters (resiste correcciones manuales).
  async getNextFolioSuggestion(invoiceType) {
    const prefix = invoiceType==="factura" ? "D-" : "R-";
    // Buscar todas las órdenes con folio del tipo solicitado
    const {data, error} = await supabase.from("orders")
      .select("invoice_folio")
      .like("invoice_folio", prefix+"%");
    if(error || !data) return prefix+"1";
    // Extraer números y calcular máximo
    let maxNum = 0;
...
export default function PrintFlow() {
  ...
  const handleAction=useCallback((id,action,payload)=>{
    ...
    if(action==="pre_invoice"){const o=orders.find(x=>x.id===id);if(!o)return;
      ...
      if(!o.production_number){showToast("❌ La orden no tiene P-XXXX. Asígnalo antes de facturar.","error");return}
