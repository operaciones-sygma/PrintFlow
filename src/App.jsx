...
@media print{body{padding:10px 14px}@page{margin:8mm}}
    ...
    h+=`<div class="header">
      ...
      <div class="header-title"><div class="main">Orden de Producción</div><div class="sub">Padilla Hnos. Impresora · León, Gto.</div></div>
    ...
    // ═══ DATOS DEL CLIENTE ═══
    h+=`<table style="margin-top:-1px"><tr><td colspan="3" class="section-title">Datos del Cliente</td></tr>
    <tr><td style="width:45%"><div class="field-lbl">Nombre o Denominación</div><div class="field-val" style="font-size:14px">${o.client||"—"}</div></td>
    <td style="width:35%"><div class="field-lbl">Empresa</div><div class="field-val">${o.client_company||""}</div></td>
    <td style="width:20%"><div class="field-lbl">Teléfono</div><div class="field-val">${o.client_phone?(o.client_lada||"+52")+" "+o.client_phone:""}</div></td></tr></table>`;
    ...
    <tr>
      ...
      <td style="width:40%"><div class="field-lbl">Nombre del Producto</div><div class="field-val" style="font-size:13px">${o.product||o.product_type||""}</div></td>
      <td style="width:25%"><div class="field-lbl">Tipo de Producto</div><div class="field-val">${o.product_type||""}</div></td>
      <td style="width:20%"><div class="field-lbl">Tamaño Final</div><div class="field-val">${o.width_cm?o.width_cm+" × "+o.height_cm+" cm":""}</div></td>
    </tr></table>`;

    // ═══ IMPRESIÓN ═══
    h+=`<table class="imp-table" style="margin-top:-1px">
    <tr><td colspan="6" class="section-title">Impresión</td></tr>
    <tr class="hdr"><td class="hdr" style="width:25%">Tipo de Papel</td><td class="hdr" style="width:10%">Gramaje</td><td class="hdr" style="width:20%">Tintas Frente</td><td class="hdr" style="width:20%">Tintas Vuelta</td><td class="hdr" style="width:15%">Medidas Pliego</td><td class="hdr" style="width:10%">Color</td></tr>
    <tr><td style="font-weight:600">${o.paper_type||""}</td><td style="text-align:center;font-weight:600">${o.paper_grammage?o.paper_grammage+" grs":""}</td><td style="font-weight:600">${o.ink_front||o.colors||""}</td><td style="font-weight:600">${o.ink_back||""}</td><td style="text-align:center">${o.width_cm?o.width_cm+" × "+o.height_cm:""}</td><td></td></tr>
    </table>`;
    ...
    <td style="width:15%"><div class="field-lbl">Precio</div><div class="field-val" style="font-weight:800;font-size:14px">${hp?"":o.price?fmt(o.price):""}</div></td></tr>
...
function DetailModal({order:o,onClose,onPrint,role}) {
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998}} onClick={onClose}>
    <div style={{background:C.bg,borderRadius:20,padding:24,maxWidth:520,width:"92%",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      ...
      {(o.notes_log||[]).length>0&&<><div style={{fontSize:10,fontWeight:600,color:"#007aff",textTransform:"uppercase",marginTop:12,marginBottom:4}}>💬 Notas Rápidas ({o.notes_log.length})</div><div style={{maxHeight:120,overflowY:"auto"}}>{(o.notes_log||[]).map((n,i)=>{const rc={secretaria:"#5856d6",produccion:"#007aff",preprensa:"#ec4899",german:"#0891b2",admin:C.ok};const rN={produccion:"Producción",preprensa:"Noemí",german:"Germán",secretaria:"Secretaría",admin:"Admin"};return <div key={i} style={{padding:"4px 0",borderBottom:i<o.notes_log.length-1?"0.5px solid "+C.bd:"none"}}><span style={{fontSize:10,fontWeight:600,color:rc[n.by]||C.t3}}>{rN[n.by]||n.by}</span> <span style={{fontSize:11}}>{n.text}</span> <span style={{fontSize:9,color:C.t3}}>{fDT(n.date)}</span></div>})}</div></>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>
      {days.map((day,i)=>{const ds=day.getFullYear()+"-"+String(day.getMonth()+1).padStart(2,"0")+"-"+String(day.getDate()).padStart(2,"0");const isT=day.toDateString()===today.toDateString();
        ...
        return <div key={i} style={{background:isT?C.ac+"08":C.bg,border:"0.5px solid "+(isT?C.ac+"30":C.bd),borderRadius:10,padding:8,minHeight:100}}>
          ...
          {pend.map(o=>{const st=SM[o.stage];return <div key={o.id} onClick={()=>{setEditOrder(o);setNewDate(o.due_date)}} style={{background:isLate?C.dn+"10":C.ac+"08",borderRadius:8,padding:"5px 7px",marginBottom:4,borderLeft:"3px solid "+(o.priority==="urgente"?C.dn:isLate?C.dn:C.ac),cursor:"pointer",transition:"all .15s"}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,.1)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
...
// ─── WEEKLY REPORT ─────────────────────────────────
function WeeklyReport({orders,role,chemicals=[],plates=[],maintenance=[]}) {
...
function OrderForm({role,onSubmit,editOrder,onCancel,clients}) {
  ...
  return <div style={{background:C.sf,borderRadius:16,overflow:"hidden",maxWidth:700,margin:"0 auto"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:"0.5px solid "+C.bd}}><FC label="Tipo" req br><select style={{...inp,paddingRight:32}} value={PTYPES.includes(f.product_type)?f.product_type:"Otro"} onChange={e=>{if(e.target.value==="Otro"){s("product_type","Otro")}else{s("product_type",e.target.value)}}}>{PTYPES.map(t=><option key={t}>{t}</option>)}</select>{(!PTYPES.includes(f.product_type)||f.product_type==="Otro")&&<input style={{...inp,marginTop:6}} value={f.product_type==="Otro"?"":f.product_type} onChange={e=>s("product_type",e.target.value||"Otro")} placeholder="Escribe el tipo de producto"/>}</FC><FC label="Descripción del producto"><textarea style={{...inp,minHeight:52,resize:"vertical"}} value={f.product} onChange={e=>s("product",e.target.value)} placeholder="Ej: Revista 2 hojas a color, 5 hojas blanco y negro, portada couché..."/></FC></div>
    {!specsOnly&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",borderBottom:"0.5px solid "+C.bd}}><FC label="Cantidad" rec br><input style={inp} type="number" value={f.quantity} onChange={e=>s("quantity",e.target.value)} placeholder="Tiraje"/></FC><FC label="📅 Entrega" rec br><input style={inp} type="date" value={f.due_date} onChange={e=>s("due_date",e.target.value)}/></FC><FC label="# Producción"><div style={{display:"flex",alignItems:"center",gap:0}}><span style={{padding:"10px 8px 10px 14px",background:"#fff",borderRadius:"12px 0 0 12px",fontSize:13,fontWeight:700,color:C.ac,boxShadow:"0 0 0 0.5px rgba(0,0,0,0.06)"}}>P-</span><input style={{...inp,borderRadius:"0 12px 12px 0",paddingLeft:6}} value={f.production_number?.replace(/^P-/,"")||""} onChange={e=>{const v=e.target.value.replace(/\D/g,"").slice(0,4);s("production_number",v?"P-"+v:"")}} placeholder="0001" maxLength={4}/></div></FC></div>}
    ...
    <div style={{padding:"12px 20px",borderBottom:"0.5px solid "+C.bd}}><label style={lbl}>📝 Notas de Proceso / Aclaraciones</label><textarea style={{...inp,minHeight:48,resize:"vertical"}} value={f.notes} onChange={e=>s("notes",e.target.value)} placeholder="Ej: No hay archivo, el cliente lo manda directo a Noemí. Entregar en 2 paquetes separados..."/></div>
...
// ─── ORDER CARD ────────────────────────────────────
function OCard({o,role,onAction,compact,busy,noDragHint}) {
  ...
  return <div draggable={isDraggable} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    {!compact&&<div onClick={e=>e.stopPropagation()}><QuickNotes notes={o.notes_log||[]} onAdd={text=>onAction(o.id,"quick_note",text)} role={role}/></div>}
  </div>;
}
...
function Archive({orders,role,onAction}) {
  const MONTHS=["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"];
  ...
  const totalRev=delivered.reduce((s,o)=>s+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0);
  ...
  return <div>
    ...
    {years.map(y=>{
      ...
      const yRev=months.reduce((s,m)=>s+Object.values(tree[y][m]).reduce((s2,w)=>s2+w.reduce((s3,o)=>s3+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0),0),0);
      ...
      return <div key={y} style={{marginBottom:8}}>
        ...
        {yOpen&&<div style={{paddingLeft:12,marginTop:4}}>
          {months.map(m=>{
            ...
            const mRev=weeks.reduce((s,w)=>s+tree[y][m][w].reduce((s2,o)=>s2+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0),0);
            ...
            return <div key={m} style={{marginBottom:4}}>
              ...
              {mOpen&&<div style={{paddingLeft:12,marginTop:4}}>
                {weeks.map(w=>{
                  ...
                  const wRev=wOrders.reduce((s,o)=>s+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0);
                      ...
                      {wOrders.map(o=>{
                        ...
                        return <div key={o.id} onClick={()=>onAction(o.id,"detail")} style={{background:C.bg,borderRadius:10,padding:10,cursor:"pointer",borderLeft:"3px solid "+(st?.c||C.ok),boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4}}>
                            ...
                            {!hp&&(o.price||o.maq_price)&&<span style={{fontSize:12,fontWeight:800,color:C.ok}}>{fmt(o.price||o.maq_price)}</span>}
                          </div>
                        </div>;
                      })}
                    </div>}
...
export default function PrintFlow() {
  ...
  const create=useCallback(async f=>{
    ...
    const newOrder={...f,id:gid(),stage:isMaq?"maq_created":"draft",priority:f.priority||"normal",created_at:new Date().toISOString(),created_by:user,validated_by_production:false,validated_by_preprensa:false,price:f.price?parseFloat(f.price):null,quantity:f.quantity?parseInt(f.quantity,10):null,estimated_hours:f.estimated_hours?parseFloat(f.estimated_hours):null,maq_cost:f.maq_cost?parseFloat(f.maq_cost):null,maq_price:f.maq_price?parseFloat(f.maq_price):null,machine_log:[],waste_log:[],comments:[],notes_log:[],current_machine:null,proof_approved:null,timeline:[{action:"📋 Orden creada",date:new Date().toISOString(),by:user,color:C.ac}]};
  ...
  const duplicate=useCallback(async id=>{
    ...
    const dup={...orig,id:gid(),stage:orig.order_type==="maquila"?"maq_created":"draft",created_at:new Date().toISOString(),created_by:user,validated_by_production:false,validated_by_preprensa:false,machine_log:[],waste_log:[],comments:[],notes_log:[],current_machine:null,proof_approved:null,deliveredAt:null,delivered_at:null,maquila_provider:null,maquila_phone:null,maquila_email:null,file_url:null,file_name:null,timeline:[{action:"📋 Duplicada de "+orig.id,date:new Date().toISOString(),by:user,color:"#5856d6"}]};
    setOrders(p=>[dup,...p]);
    try{
  ...
  const handleAction=useCallback((id,action,payload)=>{
    ...
    if(action==="quick_note"){(async()=>{try{await db.addNote(id,payload,user);setOrders(p=>p.map(o=>o.id===id?{...o,notes_log:[...(o.notes_log||[]),{text:payload,by:user,date:new Date().toISOString()}]}:o))}catch(e){showToast("❌ Error al enviar nota","error")}})()}
  ...
  },[orders,advance,approveProof,addComment,duplicate,deleteOrder,doAdv,showToast]);
  ...
  const searchFilter=useCallback(o=>{if(!search)return true;const q=search.toLowerCase();return[o.client,o.product,o.id,o.product_type,o.maq_provider,o.maquila_provider,o.client_company,o.production_number,o.agent,o.notes,o.paper_type,o.finishes].some(x=>x?.toLowerCase().includes(q))},[search]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        ...
        {view==="orders"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase"}}>Todas ({orders.length}){search&&<span style={{fontSize:13,fontWeight:500,color:C.t2,textTransform:"none"}}> · filtro: "{search}"</span>}</h2>{orders.filter(searchFilter).sort(prioSort).map(o=><OCard key={o.id} o={o} role={user} onAction={handleAction} busy={actionLoading===o.id} noDragHint/>)}</div>}
        {view==="archive"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>🗂️ Archivo de Completadas</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Órdenes entregadas organizadas por fecha</p><Archive orders={orders} role={user} onAction={handleAction}/></div>}
