...
export default function PrintFlow() {
  ...
  const viewOrders=useMemo(()=>{if(!hasFilter||orderFilter==="all")return orders;return orders.filter(o=>!o.created_by||o.created_by===userLogin)},[orders,orderFilter,userLogin,hasFilter]);

  const myTasks=useMemo(()=>{const srt=l=>l.sort(prioSort);const isFinal=s=>s.includes("delivered")||s.includes("cancelled");if(user==="admin")return srt(viewOrders.filter(o=>!isFinal(o.stage)));if(user==="produccion")return srt(viewOrders.filter(o=>["draft","ready","in_production","maquila_out","maquila_in","packaging","placas_listas"].includes(o.stage)));if(user==="preprensa")return srt(viewOrders.filter(o=>["draft","design","proof_client"].includes(o.stage)));if(user==="german")return srt(viewOrders.filter(o=>["proof_printing","ctp"].includes(o.stage)));if(isSec(user))return srt(viewOrders.filter(o=>["draft","proof_client","maq_created","maq_sent","maq_in_progress","maq_received","salidas"].includes(o.stage)));return []},[viewOrders,user]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        {view==="pipeline"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Dashboard</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{viewOrders.length} órdenes · {viewOrders.filter(o=>!o.stage.includes("delivered")&&!o.stage.includes("cancelled")).length} activas{hasFilter&&orderFilter==="mine"?" (mis órdenes)":""}</p>{(user==="admin"||isSec(user))&&<WeeklyReport orders={viewOrders} role={user} chemicals={chemicals} plates={plates} maintenance={maintenance} userLogin={userLogin}/>}<MaquilaTracker orders={viewOrders} onAction={handleAction} role={user} userLogin={userLogin}/><Pipeline orders={viewOrders} role={user} onAction={handleAction}/></div>}
        ...
        {view==="board"&&user==="german"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Tablero Germán</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Arrastra órdenes a CTP y Procesadora · ⠿ para mover</p><PreprensaBoard orders={viewOrders} onDrop={assignMachine} onAction={handleAction} onPlateRequired={(oid,mid,o,m)=>setPlateModal({oid,mid,order:o,machine:m})} maintenance={maintenance}/></div>}
        {view==="board"&&(user==="produccion"||user==="admin")&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Tablero de Producción</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Arrastra órdenes entre máquinas · ⠿ para mover</p><Kanban orders={viewOrders} onDrop={assignMachine} onAction={handleAction} role={user} maintenance={maintenance} onMaintenance={(type,machine,record)=>setMaintModal({type,machine,record})}/><MaquilaTracker orders={viewOrders} onAction={handleAction} role={user} userLogin={userLogin}/>{user==="admin"&&<><h3 style={{fontSize:15,fontWeight:800,margin:"20px 0 4px",textTransform:"uppercase",color:"#0891b2"}}>💿 Tablero Germán</h3><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>CTP y Procesadora</p><PreprensaBoard orders={viewOrders} onDrop={assignMachine} onAction={handleAction} onPlateRequired={(oid,mid,o,m)=>setPlateModal({oid,mid,order:o,machine:m})} maintenance={maintenance}/></>}</div>}
        {view==="board"&&isSec(user)&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Salidas</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{orderFilter==="mine"?"Tus órdenes listas para entregar":"Todas las órdenes listas para entregar"}</p>{(()=>{const sal=viewOrders.filter(o=>o.stage==="salidas");return sal.length===0?<div style={{textAlign:"center",padding:"40px 20px",color:C.t3}}><div style={{fontSize:48}}>📤</div><div style={{fontSize:15,fontWeight:700,color:C.tx,marginTop:8}}>Sin órdenes en salida</div><div style={{fontSize:12,color:C.t2,marginTop:4}}>Las órdenes aparecerán aquí cuando Producción las envíe</div></div>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>{sal.sort(prioSort).map(o=>{const canDeliver=!o.created_by||o.created_by===userLogin||user==="admin";const isOwn=user!=="vendedor"||!o.created_by||o.created_by===userLogin;return <div key={o.id} onClick={()=>handleAction(o.id,"detail")} style={{background:C.bg,borderRadius:14,padding:16,cursor:"pointer",borderLeft:"4px solid #16a34a",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:14,fontWeight:700}}>{o.client}{isOwn&&o.client_company?" · "+o.client_company:""}</div><div style={{fontSize:11,color:C.t2,marginTop:2}}>{o.product_type}{o.quantity?" · "+Number(o.quantity).toLocaleString()+" pzas":""}</div>{o.due_date&&<div style={{fontSize:10,color:new Date(o.due_date)<new Date()?C.dn:C.t3,marginTop:4}}>📅 Entrega: {fD(o.due_date)}</div>}{isOwn&&o.price&&<div style={{fontSize:13,fontWeight:700,color:C.ok,marginTop:4}}>{fmt(o.price)}</div>}{canDeliver&&<button onClick={e=>{e.stopPropagation();handleAction(o.id,"advance","delivered")}} style={{...bt(C.ok),marginTop:10,width:"100%",justifyContent:"center"}}>✅ Marcar Entregada</button>}</div>})}</div>})()}<MaquilaTracker orders={viewOrders} onAction={handleAction} role={user} userLogin={userLogin}/></div>}
        {view==="calendar"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase"}}>Calendario de Entregas</h2><Calendar orders={viewOrders} onChangeDate={changeDate} role={user} userLogin={userLogin}/></div>}
        {view==="orders"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase"}}>Todas ({viewOrders.length}){search&&<span style={{fontSize:13,fontWeight:500,color:C.t2,textTransform:"none"}}> · filtro: "{search}"</span>}</h2>{viewOrders.filter(searchFilter).sort(prioSort).map(o=><OCard key={o.id} o={o} role={user} onAction={handleAction} busy={actionLoading===o.id} noDragHint userLogin={userLogin}/>)}</div>}
        {view==="archive"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>🗂️ Archivo de Completadas</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Órdenes entregadas organizadas por fecha</p><Archive orders={viewOrders} role={user} onAction={handleAction} userLogin={userLogin}/></div>}
        {view==="analytics"&&user==="admin"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase",textAlign:"center"}}>Analytics</h2><Analytics orders={viewOrders} onReload={reload}/></div>}
