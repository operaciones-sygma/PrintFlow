...
@media print{body{padding:10px 14px}@page{margin:8mm}}
  ...
  const canChoose=role==="secretaria"||role==="admin";
                      ...
                      {wOrders.map(o=>{
                        ...
                        return <div key={o.id} onClick={()=>onAction(o.id,"detail")} style={{background:C.bg,borderRadius:10,padding:10,cursor:"pointer",borderLeft:"3px solid "+(st?.c||C.ok),boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4}}>
                            ...
                            {!hp&&(o.price||o.maq_price)&&<span style={{fontSize:12,fontWeight:800,color:C.ok}}>{fmt(parseFloat(o.price)||parseFloat(o.maq_price))}</span>}
                          </div>
                        </div>;
                      })}
                    </div>}
...
export default function PrintFlow() {
  ...
  const duplicate=useCallback(async id=>{
    ...
    const dup={...orig,id:gid(),stage:orig.order_type==="maquila"?"maq_created":"draft",created_at:new Date().toISOString(),created_by:user,validated_by_production:false,validated_by_preprensa:false,production_number:null,machine_log:[],waste_log:[],comments:[],notes_log:[],current_machine:null,proof_approved:null,deliveredAt:null,delivered_at:null,maquila_provider:null,maquila_phone:null,maquila_email:null,file_url:null,file_name:null,timeline:[{action:"📋 Duplicada de "+orig.id,date:new Date().toISOString(),by:user,color:"#5856d6"}]};
    setOrders(p=>[dup,...p]);
    try{
  ...
  },[orders,advance,approveProof,addComment,duplicate,deleteOrder,doAdv,showToast,user,reload]);
