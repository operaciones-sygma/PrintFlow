...
const GUIDES={produccion:{draft:"Revisa las specs y valida. Pre-prensa también debe validar",ready:"Arrastra al Tablero para asignar máquina",in_production:"Arrastra a otra máquina, Empaque o Maquila",packaging:"Arrastra a Salidas o envía a Maquila",maquila_in:"Arrastra a una máquina de acabados o Empaque"},preprensa:{draft:"Revisa y edita las specs si es necesario. Valida cuando estén correctas",design:"Noemí: prepara los archivos. Decide si necesita prueba de color",proof_printing:"Imprime prueba en Epson P7570",proof_client:"Esperando que el cliente apruebe",ctp:"Arrastra a CTP y Procesadora en el Tablero Pre-prensa"},secretaria:{maq_created:"Envía al proveedor",maq_sent:"Da seguimiento al proveedor",salidas:"Confirma la entrega al cliente"}};
...
function NotificationTray({notifications,onClose,onRead,onReadAll,role}) {
  const rc={produccion:"#007aff",preprensa:"#ec4899",secretaria:"#5856d6",admin:C.ok,sistema:C.t3};
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:9,color:C.t3,marginTop:3}}>
                <span style={{color:rc[n.by_user]||C.t3,fontWeight:600}}>{n.by_user}</span> · {fDT(n.created_at)}
              </div>
            </div>
            {!n.read&&<div style={{width:8,height:8,borderRadius:"50%",background:"#007aff",flexShrink:0,marginTop:4}}/>}
          </div>
        </div>)}
      </div>
    </div>
...
function OCard({o,role,onAction,compact}) {
  ...
  return <div draggable={["ready","in_production","maquila_in","packaging"].includes(o.stage)} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    {!compact&&canAct&&<div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>
      ...
      {o.stage==="maquila_in"&&<><button onClick={()=>onAction(o.id,"advance","ready")} style={bt("#007aff")}>🔄 Volver a Producción</button><button onClick={()=>onAction(o.id,"advance","packaging")} style={bt("#af52de")}>📦 Empaque</button></>}
    ...
    {!compact&&role==="secretaria"&&o.order_type!=="maquila"&&<div onClick={e=>e.stopPropagation()} style={{marginTop:6}}>
      ...
      {(o.stage!=="draft"&&o.stage!=="salidas"||(o.validated_by_production&&o.validated_by_preprensa&&o.stage==="draft"))&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:"#5856d608",borderRadius:10,border:"1px solid #5856d620"}}>
...
// ─── PREPRENSA BOARD ──────────────────────────────
function PreprensaBoard({orders,onDrop,onAction}) {
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
      {ppMachines.map(m=>{const mo=assigned.filter(o=>o.current_machine===m.id);const isD=dO===m.id;const hasWork=mo.length>0;
        return <div key={m.id} onDragOver={e=>{e.preventDefault();setDO(m.id)}} onDragLeave={()=>setDO(null)} onDrop={e=>drop(m.id,e)}
          style={{background:isD?"#0891b212":C.bg,borderRadius:14,padding:16,border:isD?"2px solid #0891b2":hasWork?"1.5px solid #0891b240":"1.5px dashed "+C.bd,minHeight:120,transition:"all .15s",boxShadow:hasWork?"0 2px 8px #0891b215":"none"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,paddingBottom:8,borderBottom:"0.5px solid "+C.bd}}>
            <div>
              <div style={{fontSize:15,fontWeight:700,color:C.tx}}>{m.name}</div>
              <div style={{fontSize:10,color:"#0891b2",fontWeight:500}}>{m.sub}</div>
            </div>
          ...
          :mo.map(o=><div key={o.id} draggable onDragStart={e=>e.dataTransfer.setData("orderId",o.id)} onClick={()=>onAction(o.id,"detail")}
            ...
            <div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:4,marginTop:6}}>
              <button onClick={()=>onAction(o.id,"advance","ready")} style={bs(C.ok)}>✅ Placas Listas</button>
              <button onClick={()=>onAction(o.id,"waste")} style={{...bs(C.sf,C.t2),padding:"4px 8px",boxShadow:"0 0 0 0.5px "+C.bd}}>🗑️</button>
            </div>
          </div>)}
        </div>})}
...
export default function PrintFlow() {
  ...
  navs.push({id:"board",i:"🏭",l:"Tablero"});
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        ...
        {view==="board"&&user==="preprensa"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Tablero Pre-prensa</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Arrastra órdenes a CTP y Procesadora · ⠿ para mover</p><PreprensaBoard orders={orders} onDrop={assignMachine} onAction={handleAction}/></div>}
        {view==="board"&&(user==="produccion"||user==="admin")&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Tablero de Producción</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Arrastra órdenes entre máquinas · ⠿ para mover</p><Kanban orders={orders} onDrop={assignMachine} onAction={handleAction} role={user}/><MaquilaTracker orders={orders} onAction={handleAction} role={user}/>{user==="admin"&&<><h3 style={{fontSize:15,fontWeight:800,margin:"20px 0 4px",textTransform:"uppercase",color:"#0891b2"}}>💿 Tablero Pre-prensa</h3><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>CTP y Procesadora</p><PreprensaBoard orders={orders} onDrop={assignMachine} onAction={handleAction}/></>}</div>}
      ...
      {showNotifs&&<NotificationTray notifications={notifications} onClose={()=>setShowNotifs(false)} onRead={async id=>{await db.markRead(id);setNotifications(p=>p.map(n=>n.id===id?{...n,read:true}:n))}} onReadAll={async()=>{await db.markAllRead(user);setNotifications(p=>p.map(n=>({...n,read:true})))}} role={user}/>}
