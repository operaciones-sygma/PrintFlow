...
function Timeline({tl=[]}) {
  ...
  return <div style={{marginTop:6}}><button onClick={()=>setOp(!op)} style={{...bs(C.sf,C.t2),boxShadow:"0 0 0 0.5px "+C.bd,padding:"4px 10px",fontSize:10}}>📜 ({tl.length}) {op?"▲":"▼"}</button>{(op||tl.length<=3)&&<div style={{marginTop:6,borderLeft:"2px solid "+C.bd,paddingLeft:12}}>{show.map((e,i)=><div key={i} style={{marginBottom:5}}><div style={{fontSize:10,color:C.tx,fontWeight:500}}>{e.action}</div><div style={{fontSize:10,color:C.t3}}>{fDT(e.date)}</div></div>)}</div>}</div>;
}
      ...
      {days.map((day,i)=>{const ds=day.getFullYear()+"-"+String(day.getMonth()+1).padStart(2,"0")+"-"+String(day.getDate()).padStart(2,"0");const isT=day.toDateString()===today.toDateString();
        ...
        return <div key={i} style={{background:isT?C.ac+"08":C.bg,border:"0.5px solid "+(isT?C.ac+"30":C.bd),borderRadius:10,padding:8,minHeight:90}}>
          ...
          {pend.map(o=><div key={o.id} onClick={()=>{setEditOrder(o);setNewDate(o.due_date)}} style={{background:day<today?C.dn+"10":C.ac+"08",borderRadius:6,padding:"3px 6px",marginBottom:3,borderLeft:"2px solid "+(day<today?C.dn:C.ac),cursor:"pointer"}} title="Click para cambiar fecha">
            ...
            <div style={{fontSize:10,color:C.t2}}>{o.product_type}</div>
          </div>)}
  ...
  useEffect(() => {
    ...
    orders.forEach(o => {
      if (o.stage === "draft" && o.validated_by_production && o.validated_by_preprensa && !autoFixRef.current.has(o.id)) {
        ...
        (async () => {
          const { count } = await supabase.from("orders").update({ stage: "design" }, { count: "exact" }).eq("id", o.id).eq("stage", "draft");
