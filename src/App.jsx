...
@media print{body{padding:10px 14px}@page{margin:8mm}}
    ...
    h+=`<div class="header">
      ...
      <div class="header-folio"><div class="lbl">Folio</div><div class="num">${o.production_number||o.id}</div>${o.cart_folio?'<div class="cart">🛒 '+o.cart_folio+'</div>':''}${o.web_folio?'<div class="webf">'+o.web_folio+'</div>':''}<div class="date">${pDate.getDate()} ${months[pDate.getMonth()].slice(0,3)} ${pDate.getFullYear()}</div></div>
    </div>`;
...
export default function PrintFlow() {
  ...
  const create=useCallback(async f=>{
    ...
    setView("pipeline");
    }catch(e){console.error("[create] Error al crear orden:",e);showToast("❌ No se pudo crear: "+(e?.message||"error desconocido"),"error");reload()}
  ...
  const update=useCallback(async f=>{
    ...
    }catch(e){console.error("[update] Error al guardar:",e);showToast("❌ No se pudo guardar: "+(e?.message||"error desconocido"),"error");reload()}
