...
const db = {
  async loadOrders() {
    ...
    return orders.map(o => ({
      ...
      notes_log: (nl.data || []).filter(n => n.order_id === o.id).map(n => ({ text: n.text, by: n.by_user, date: n.created_at, _id: n.id })),
    }));
  },
  async saveOrder(o) {
...
function Timeline({tl=[]}) {
  ...
  const rc={secretaria:"#5856d6",vendedor:"#d97706",produccion:"#007aff",preprensa:"#ec4899",german:"#0891b2",sistema:C.t3,admin:C.ok};
  const rN={produccion:"Producción",preprensa:"Noemí",german:"Germán",secretaria:"Secretaría",vendedor:"Vendedor",admin:"Admin",sistema:"Sistema"};
...
@media print{body{padding:10px 14px}@page{margin:8mm}}
    ...
    h+=`<div class="header">
      ...
      <div class="header-folio"><div class="lbl">Folio</div><div class="num">${o.production_number||o.id}</div><div class="date">${pDate.getDate()} ${months[pDate.getMonth()].slice(0,3)} ${pDate.getFullYear()}</div></div>
    </div>`;
    ...
    <tr>
      ...
      <td style="width:30%"><div class="field-lbl">Tamaño Final</div><div class="field-val">${o.width_cm?o.width_cm+" × "+o.height_cm+" cm":""}</div></td>
      <td style="width:20%"><div class="field-lbl">Tipo de Orden</div><div class="field-val">${isMaq?"Maquila":"Interna"}</div></td>
    </tr>
    ${o.product?`<tr><td colspan="4"><div class="field-lbl">Descripción del Producto</div><div class="field-val" style="font-size:12px;line-height:1.6;white-space:pre-wrap;min-height:24px">${o.product}</div></td></tr>`:""}
    </table>`;
...
function DetailModal({order:o,onClose,onPrint,role,userLogin}) {
  useEscClose(onClose);
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998}} onClick={onClose}>
    <div style={{background:C.bg,borderRadius:20,padding:24,maxWidth:520,width:"92%",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"flex",gap:8,marginTop:16}}>
        ...
        {vOwns&&<button onClick={printIt} style={{...bt(C.ac),flex:1,justifyContent:"center"}}>🖨️ Imprimir Orden</button>}
      </div>
    </div>
  </div>;
}
  ...
  return <div style={{background:C.sf,borderRadius:16,padding:16}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>
      {days.map((day,i)=>{const ds=day.getFullYear()+"-"+String(day.getMonth()+1).padStart(2,"0")+"-"+String(day.getDate()).padStart(2,"0");const isT=day.toDateString()===today.toDateString();
        ...
        return <div key={i} style={{background:isT?C.ac+"08":C.bg,border:"0.5px solid "+(isT?C.ac+"30":C.bd),borderRadius:10,padding:8,minHeight:100}}>
          ...
          {pend.map(o=>{const st=SM[o.stage];const canEdit=canEditDate(o);return <div key={o.id} onClick={()=>{if(canEdit){setEditOrder(o);setNewDate(o.due_date)}}} style={{background:isLate?C.dn+"10":C.ac+"08",borderRadius:8,padding:"5px 7px",marginBottom:4,borderLeft:"3px solid "+(o.priority==="urgente"?C.dn:isLate?C.dn:C.ac),cursor:canEdit?"pointer":"default",transition:"all .15s",opacity:canEdit?1:0.7}} onMouseEnter={e=>{if(canEdit)e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,.1)"}} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
            <div style={{fontSize:10,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{o.client}</div>
            <div style={{fontSize:9,color:C.t2,marginTop:1}}>{o.product_type}{o.quantity?" · "+Number(o.quantity).toLocaleString()+" pzas":""}</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:2}}>
              <span style={{fontSize:8,color:st?.c,fontWeight:600}}>{st?.l}</span>
              {o.priority==="urgente"&&<span style={{fontSize:7,color:C.dn,fontWeight:800}}>🔴 URG</span>}
            </div>
          </div>})}
          {done.map(o=><div key={o.id} style={{background:C.ok+"08",borderRadius:8,padding:"4px 7px",marginBottom:4,borderLeft:"3px solid "+C.ok,opacity:.6}}>
            <div style={{fontSize:9,color:C.ok,fontWeight:600}}>✓ {o.client}</div>
            <div style={{fontSize:8,color:C.t3}}>{o.product_type}</div>
          </div>)}
          {!pend.length&&!done.length&&<div style={{color:C.ph,fontSize:9,textAlign:"center",marginTop:20}}>—</div>}
        </div>})}
    </div>
...
function WeeklyReport({orders,role,chemicals=[],plates=[],maintenance=[],userLogin}) {
  ...
  const myOrders=role==="vendedor"?orders.filter(o=>!o.created_by||o.created_by===userLogin):orders;
  ...
  const late=myOrders.filter(o=>o.due_date&&new Date(o.due_date)<now&&!o.stage.includes("delivered")).length;
...
function OrderForm({role,onSubmit,editOrder,onCancel,clients,orders=[]}) {
  ...
  const [f,setF]=useState(editOrder?{...empty,...Object.fromEntries(Object.entries(editOrder).map(([k,v])=>[k,v===null&&typeof empty[k]==="string"?"":v]))}:empty);const [saving,setSaving]=useState(false);const [showOtroFinish,setShowOtroFinish]=useState(false);
  ...
  useEffect(()=>{if(editOrder){setF({...empty,...Object.fromEntries(Object.entries(editOrder).map(([k,v])=>[k,v===null&&typeof empty[k]==="string"?"":v]))});const fins=(editOrder.finishes||"").split(",").map(s=>s.trim()).filter(Boolean);setShowOtroFinish(fins.some(x=>!FINISHES.includes(x)&&x!=="Otro"))}},[editOrder]);
  ...
  return <div style={{background:C.sf,borderRadius:16,overflow:"hidden",maxWidth:700,margin:"0 auto"}}>
    ...
    <div style={{borderBottom:"0.5px solid "+C.bd}}><FC label="📝 Descripción del producto"><textarea style={{...inp,minHeight:90,resize:"vertical",lineHeight:1.6}} value={f.product} onChange={e=>s("product",e.target.value)} placeholder="Describe el producto con detalle: Revista 2 hojas a color, 5 hojas blanco y negro, portada couché 250g, interiores bond 90g, grapada al centro, tamaño carta..."/></FC></div>
    {!specsOnly&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",borderBottom:"0.5px solid "+C.bd}}><FC label="Cantidad" rec br><input style={inp} type="number" value={f.quantity} onChange={e=>s("quantity",e.target.value)} placeholder="Tiraje"/></FC><FC label="📅 Entrega" rec br><input style={inp} type="date" value={f.due_date} onChange={e=>s("due_date",e.target.value)}/></FC><FC label="# Producción"><div style={{display:"flex",alignItems:"center",gap:0}}><span style={{padding:"10px 8px 10px 14px",background:"#fff",borderRadius:"12px 0 0 12px",fontSize:13,fontWeight:700,color:C.ac,boxShadow:"0 0 0 0.5px rgba(0,0,0,0.06)"}}>P-</span><input style={{...inp,borderRadius:"0 12px 12px 0",paddingLeft:6}} value={f.production_number?.replace(/^P-/,"")||""} onChange={e=>{const v=e.target.value.replace(/\D/g,"").slice(0,4);s("production_number",v?"P-"+v:"")}} placeholder={nextPN?.replace(/^P-/,"")||"0001"} maxLength={4}/></div>{!editOrder&&<div style={{marginTop:4,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap"}}>{lastPN&&<span style={{fontSize:9,color:C.t3}}>Último: <strong style={{color:C.t2}}>{lastPN}</strong></span>}<button type="button" onClick={()=>s("production_number",nextPN)} style={{background:nextPN===f.production_number?C.ok+"15":"#007aff10",color:nextPN===f.production_number?C.ok:"#007aff",border:"none",borderRadius:6,padding:"2px 8px",fontSize:9,fontWeight:600,cursor:"pointer",fontFamily:"'Poppins',sans-serif"}}>{nextPN===f.production_number?"✓ "+nextPN:"→ Usar "+nextPN}</button>{f.production_number&&f.production_number!==nextPN&&orders.some(o=>o.production_number===f.production_number&&o.id!==editOrder?.id)&&<span style={{fontSize:9,color:C.dn,fontWeight:600}}>⚠️ Ya existe</span>}</div>}</FC></div>}
    ...
    <div style={{padding:"12px 20px",borderBottom:"0.5px solid "+C.bd}}><label style={lbl}>📝 Notas de Proceso / Aclaraciones <span style={{color:C.wn,fontSize:9,fontWeight:500}}>(USO INTERNO)</span></label><textarea style={{...inp,minHeight:48,resize:"vertical"}} value={f.notes} onChange={e=>s("notes",e.target.value)} placeholder="Ej: No hay archivo, el cliente lo manda directo a Noemí. Entregar en 2 paquetes separados..."/></div>
...
// ─── ORDER CARD ────────────────────────────────────
function OCard({o,role,onAction,compact,busy,noDragHint,userLogin}) {
  ...
  return <div draggable={isDraggable} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    {isDraggable&&!compact&&!noDragHint&&<div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,padding:"4px 8px",background:C.ac+"08",borderRadius:8,border:"1px dashed "+C.ac+"30"}}><span style={{fontSize:14,color:C.ac}}>⠿</span><span style={{fontSize:10,color:C.ac,fontWeight:600}}>Arrastra al Tablero</span></div>}
    ...
    {!compact&&canAct&&<div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap",opacity:busy?0.5:1,pointerEvents:busy?"none":"auto",position:"relative"}}>
      ...
      {o.stage==="salidas"&&(role==="admin"||(isSec(role)&&(!o.created_by||o.created_by===userLogin)))&&<button onClick={()=>onAction(o.id,"advance","delivered")} style={bt(C.ok)}>✅ Marcar Entregada</button>}
    ...
    {!compact&&isSec(role)&&(!o.created_by||o.created_by===userLogin)&&o.order_type==="maquila"&&!o.stage.includes("delivered")&&<div onClick={e=>e.stopPropagation()} style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap"}}>
    ...
    {!compact&&vOwns&&(isMaq||o.comments?.length>0)&&<div onClick={e=>e.stopPropagation()}><CommentLog comments={o.comments||[]} onAdd={c=>onAction(o.id,"comment",c)} role={role}/></div>}
    {!compact&&vOwns&&<div onClick={e=>e.stopPropagation()}><QuickNotes notes={o.notes_log||[]} onAdd={text=>onAction(o.id,"quick_note",text)} role={role}/></div>}
  </div>;
}
...
function Archive({orders,role,onAction,userLogin}) {
  ...
  const delivered=orders.filter(o=>o.stage.includes("delivered")&&(o.delivered_at||o.deliveredAt));

  // Group by year → month → week
  const tree=useMemo(()=>{
    const del=orders.filter(o=>o.stage.includes("delivered")&&(o.delivered_at||o.deliveredAt));
    const t={};
    del.forEach(o=>{
      const d=new Date(o.delivered_at||o.deliveredAt);
  ...
  const myDel=role==="vendedor"?delivered.filter(owns):delivered;
  ...
  return <div>
    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
      <div style={{background:C.sf,borderRadius:10,padding:"8px 14px",fontSize:12}}>
        <span style={{color:C.t2,fontWeight:600}}>Total completadas: </span>
    ...
    {years.map(y=>{
      ...
      const yRev=months.reduce((s,m)=>s+Object.values(tree[y][m]).reduce((s2,w)=>s2+w.filter(owns).reduce((s3,o)=>s3+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0),0),0);
      ...
      return <div key={y} style={{marginBottom:8}}>
        ...
        {yOpen&&<div style={{paddingLeft:12,marginTop:4}}>
          {months.map(m=>{
            ...
            const mRev=weeks.reduce((s,w)=>s+tree[y][m][w].filter(owns).reduce((s2,o)=>s2+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0),0);
            ...
            return <div key={m} style={{marginBottom:4}}>
              ...
              {mOpen&&<div style={{paddingLeft:12,marginTop:4}}>
                {weeks.map(w=>{
                  ...
                  const wRev=wOrders.filter(owns).reduce((s,o)=>s+(parseFloat(o.price)||parseFloat(o.maq_price)||0),0);
                  ...
                  return <div key={w} style={{marginBottom:4}}>
                    ...
                    {wOpen&&<div style={{paddingLeft:8,marginTop:4,display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:6}}>
                      {wOrders.map(o=>{
                        const st=SM[o.stage];const isMaq=o.order_type==="maquila";
                        return <div key={o.id} onClick={()=>onAction(o.id,"detail")} style={{background:C.bg,borderRadius:10,padding:10,cursor:"pointer",borderLeft:"3px solid "+(st?.c||C.ok),boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4}}>
                            <span style={{fontSize:9,color:C.t3}}>Entregada: {fD(o.delivered_at||o.deliveredAt)}</span>
                            {!hp&&owns(o)&&(o.price||o.maq_price)&&<span style={{fontSize:12,fontWeight:800,color:C.ok}}>{fmt(parseFloat(o.price)||parseFloat(o.maq_price))}</span>}
                          </div>
                        </div>;
                      })}
                    </div>}
...
export default function PrintFlow() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("orders-realtime")
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "order_notes" }, () => reload())
      .subscribe((status) => { setConnected(status==="SUBSCRIBED"); });
    return () => { supabase.removeChannel(channel); };
  }, [user, notifKey, reload]);
  ...
  useEffect(() => {
    ...
    orders.forEach(o => {
      if (o.stage === "draft" && o.validated_by_production && o.validated_by_preprensa && !autoFixRef.current.has(o.id)) {
        ...
        (async () => {
          const { count, error:afErr } = await supabase.from("orders").update({ stage: "design" }, { count: "exact" }).eq("id", o.id).eq("stage", "draft");
  ...
  useEffect(() => {
    ...
    const checkStale = () => {
      orders.forEach(o => {
        if (o.stage.includes("delivered")) return;
  ...
  const create=useCallback(async f=>{
    ...
    const newOrder={...f,id:gid(),stage:isMaq?"maq_created":"draft",priority:f.priority||"normal",created_at:new Date().toISOString(),created_by:userLogin||user,validated_by_production:false,validated_by_preprensa:false,price:f.price?parseFloat(f.price):null,quantity:f.quantity?parseInt(f.quantity,10):null,estimated_hours:f.estimated_hours?parseFloat(f.estimated_hours):null,maq_cost:f.maq_cost?parseFloat(f.maq_cost):null,maq_price:f.maq_price?parseFloat(f.maq_price):null,machine_log:[],waste_log:[],comments:[],notes_log:[],current_machine:null,proof_approved:null,timeline:[{action:"📋 Orden creada",date:new Date().toISOString(),by:user,color:C.ac}]};
    ...
    }catch(e){showToast("❌ Error al crear orden — reintenta","error");reload()}
    setView("pipeline");
  ...
  const update=useCallback(async f=>{
    ...
    }catch(e){showToast("❌ Error al guardar — reintenta","error");reload()}
  ...
  const doAdv=useCallback(async(id,ns)=>{
    ...
    if(ns==="salidas")await db.notifySecs(id,"salidas","📤 Orden de "+(o?.client||"")+" lista para entrega — "+(o?.product_type||""),null,user,o?.created_by);
    ...
    if(ns==="maq_received"&&user!=="admin")await db.addNotification("admin",id,"new_order","📥 Maquila recibida del proveedor — "+(o?.client||"")+" · "+(o?.product_type||""),null,user);
    showToast((SM[ns]?.l||"Avance")+" — "+(o?.client||""));
    }catch(e){showToast("❌ Error al avanzar orden — reintenta","error");reload()}finally{setActionLoading(null)}
  },[user,orders,showToast,reload]);
  ...
  const approveProof=useCallback(async id=>{
    ...
    }catch(e){showToast("❌ Error al aprobar prueba","error");reload()}finally{setActionLoading(null)}
  },[user,orders,showToast,reload]);
  ...
  const assignMachine=useCallback(async(oid,mid)=>{
    ...
    }catch(e){showToast("❌ Error al asignar máquina","error");reload()}
  ...
  const sendMaquila=useCallback(async(oid,prov,phone,email,note)=>{
    ...
    }catch(e){showToast("❌ Error al enviar a maquila","error");reload()}
    setMaqModal(null);
  },[user,orders,showToast,reload]);
  ...
  const deleteOrder=useCallback(async id=>{
    ...
    setConfirmModal({title:"🗑️ Borrar Orden",message:"¿Borrar \""+o.client+" — "+(o.product_type||"")+"\"?\n\nSe eliminará permanentemente con todo su historial. Esta acción NO se puede deshacer.",confirmLabel:"Sí, borrar",confirmColor:C.dn,onConfirm:async()=>{
      ...
      }catch(e){showToast("❌ Error al borrar — reintenta","error");reload()}
  ...
  const addComment=useCallback(async(oid,c)=>{
    ...
    try{await db.addComment(oid,c.text,c.by)}catch(e){showToast("❌ Error al guardar nota","error")}
  },[showToast]);
  ...
  const changeDate=useCallback(async(oid,newDate,reason)=>{
    ...
    }catch(e){showToast("❌ Error al cambiar fecha","error");reload()}
  },[user,userLogin,orders,showToast,reload]);

  const handleAction=useCallback((id,action,payload)=>{
    if(action==="edit"){const o=orders.find(x=>x.id===id);if(!o)return;if(isSec(user)&&o.created_by&&o.created_by!==userLogin)return;if(isSec(user)&&o.order_type!=="maquila"&&o.validated_by_production&&o.validated_by_preprensa)return;setEditO(o);setView("form")}
    ...
    if(action==="quick_note"){const noteObj={text:payload,by:user,date:new Date().toISOString()};setOrders(p=>p.map(o=>o.id===id?{...o,notes_log:[...(o.notes_log||[]),noteObj]}:o));(async()=>{try{await db.addNote(id,payload,user)}catch(e){showToast("❌ Error al enviar nota","error");reload()}})()}
    ...
    if(action==="validate_prod"){setActionLoading(id);(async()=>{try{setOrders(p=>p.map(o=>{if(o.id!==id)return o;const u={...o,validated_by_production:true,timeline:addTL(o,"✅ Validada por Producción")};if(o.validated_by_preprensa){u.stage="design";u.timeline=addTL(u,"📝 → 🎨 Ambos validaron → Diseño",{to:"design"})}return u}));const o=orders.find(x=>x.id===id);const {error:vpErr}=await supabase.from("orders").update({validated_by_production:true,...(o?.validated_by_preprensa?{stage:"design"}:{})}).eq("id",id);if(vpErr)throw new Error(vpErr.message);await db.addTimeline(id,"✅ Validada por Producción",user,C.ok);if(o?.validated_by_preprensa)await db.addTimeline(id,"📝 → 🎨 Ambos validaron → Diseño",user,"#ec4899");await db.notifySecs(id,"validation","✅ Producción validó la orden de "+(o?.client||"")+" — "+(o?.product_type||""),null,user,o?.created_by);showToast("✅ Orden validada por Producción")}catch(e){showToast("❌ Error al validar — reintenta","error");reload()}finally{setActionLoading(null)}})()}
    if(action==="validate_pre"){setActionLoading(id);(async()=>{try{setOrders(p=>p.map(o=>{if(o.id!==id)return o;const u={...o,validated_by_preprensa:true,timeline:addTL(o,"✅ Validada por Pre-prensa")};if(o.validated_by_production){u.stage="design";u.timeline=addTL(u,"📝 → 🎨 Ambos validaron → Diseño",{to:"design"})}return u}));const o=orders.find(x=>x.id===id);const {error:vpreErr}=await supabase.from("orders").update({validated_by_preprensa:true,...(o?.validated_by_production?{stage:"design"}:{})}).eq("id",id);if(vpreErr)throw new Error(vpreErr.message);await db.addTimeline(id,"✅ Validada por Pre-prensa",user,"#ec4899");if(o?.validated_by_production)await db.addTimeline(id,"📝 → 🎨 Ambos validaron → Diseño",user,"#ec4899");await db.notifySecs(id,"validation","✅ Pre-prensa validó la orden de "+(o?.client||"")+" — "+(o?.product_type||""),null,user,o?.created_by);showToast("✅ Orden validada por Pre-prensa")}catch(e){showToast("❌ Error al validar — reintenta","error");reload()}finally{setActionLoading(null)}})()}
    if(action==="revert"){const o=orders.find(x=>x.id===id);if(!o)return;
      ...
      if(prevId){setConfirmModal({title:"↩️ Regresar",message:"¿A \""+SM[prevId]?.l+"\"?",confirmLabel:"Sí",confirmColor:C.wn,onConfirm:async()=>{try{await doAdv(id,prevId);
        ...
        }catch(e){showToast("❌ Error al revertir","error")}setConfirmModal(null)}})}}
    if(action==="devolver_design")setDevolverModal(id);
  },[orders,advance,approveProof,addComment,duplicate,deleteOrder,doAdv,showToast,user,userLogin,reload]);

  const myTasks=useMemo(()=>{const srt=l=>l.sort(prioSort);if(user==="admin")return srt(orders.filter(o=>!o.stage.includes("delivered")));if(user==="produccion")return srt(orders.filter(o=>["draft","ready","in_production","maquila_out","maquila_in","packaging","placas_listas"].includes(o.stage)));if(user==="preprensa")return srt(orders.filter(o=>["draft","design","proof_client"].includes(o.stage)));if(user==="german")return srt(orders.filter(o=>["proof_printing","ctp"].includes(o.stage)));if(isSec(user))return srt(orders.filter(o=>(!o.created_by||o.created_by===userLogin)&&["draft","proof_client","maq_created","maq_sent","maq_in_progress","maq_received","salidas"].includes(o.stage)));return []},[orders,user,userLogin]);

  const searchFilter=useCallback(o=>{if(!search)return true;const q=search.toLowerCase();return[o.client,o.product,o.id,o.product_type,o.maq_provider,o.maquila_provider,o.client_company,o.production_number,o.agent,o.notes,o.paper_type,o.finishes,o.client_phone].some(x=>x?.toLowerCase().includes(q))},[search]);

  if(!user) return (
    <Login onLogin={(role, name, login)=>{setUser(role);setUserName(name);setUserLogin(login);setView("pipeline");ld("pf-welcome-"+role,false).then(seen=>{if(!seen){setShowWelcome(true);sv("pf-welcome-"+role,true)}})}}/>
  );

  const rL={produccion:"Producción",preprensa:"Pre-prensa",german:"Germán",secretaria:"Secretaría",vendedor:"Vendedor",admin:"Admin"};
  const rC={produccion:"#007aff",preprensa:"#ec4899",german:"#0891b2",secretaria:"#5856d6",vendedor:"#d97706",admin:C.ok};
  ...
  const navClick=(id)=>{setView(id);if(id!=="form")setEditO(null);if(!["tasks","orders"].includes(id))setSearch("");setShowMoreMenu(false)};
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{borderBottom:"0.5px solid "+C.bd,padding:"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
        ...
        <div style={{display:"flex",gap:3,alignItems:"center"}}>
          {visibleNavs.map(n=><button key={n.id} onClick={()=>navClick(n.id)} style={{background:view===n.id?C.acL:"transparent",border:"none",color:view===n.id?C.ac:C.t2,padding:"8px 14px",borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"'Poppins',sans-serif",whiteSpace:"nowrap"}}>{n.i} {n.l}</button>)}
          {moreNavs.length>0&&<div style={{position:"relative"}}>
            <button onClick={()=>setShowMoreMenu(!showMoreMenu)} style={{background:moreActive?C.acL:"transparent",border:"none",color:moreActive?C.ac:C.t2,padding:"8px 12px",borderRadius:10,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"'Poppins',sans-serif",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4}}>⋯ Más{moreActive&&<div style={{width:6,height:6,borderRadius:"50%",background:C.ac}}/>}</button>
            {showMoreMenu&&<><div style={{position:"fixed",inset:0,zIndex:997}} onClick={()=>setShowMoreMenu(false)}/><div style={{position:"absolute",top:"100%",right:0,marginTop:4,background:C.bg,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,.15)",border:"0.5px solid "+C.bd,zIndex:998,minWidth:180,overflow:"hidden"}}>
              {moreNavs.map(n=><button key={n.id} onClick={()=>navClick(n.id)} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"10px 16px",border:"none",background:view===n.id?C.acL:C.bg,color:view===n.id?C.ac:C.tx,cursor:"pointer",fontSize:12,fontWeight:view===n.id?700:500,fontFamily:"'Poppins',sans-serif",borderBottom:"0.5px solid "+C.bd,textAlign:"left"}}><span style={{fontSize:14}}>{n.i}</span>{n.l}</button>)}
            </div></>}
          </div>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {["tasks","orders"].includes(view)&&<input style={{...inp,width:160,padding:"7px 12px",fontSize:11,borderRadius:10,boxShadow:"0 0 0 0.5px "+C.bd}} placeholder="🔍 Buscar..." value={search} onChange={e=>setSearch(e.target.value)}/>}
          <NotificationBell count={notifications.filter(n=>!n.read).length} onClick={()=>setShowNotifs(!showNotifs)}/>
          {(user==="admin"||isSec(user))&&<button onClick={()=>{const csvOrders=user==="vendedor"?orders.filter(o=>!o.created_by||o.created_by===userLogin):orders;const h=["ID","Fecha","Tipo","Prioridad","#Prod","Agente","Cliente","Empresa","Tel","Email","RFC","Producto","TipoProd","Cant","Papel","Gramaje","Ancho","Alto","Tintas","TintasFrente","TintasVuelta","Acabados","Hrs","Precio","CostoMaq","PrecioMaq","Margen","Proveedor","ProvTel","ProvEmail","Etapa","Entrega","PlMerma","PzMerma","MinMaq","Prueba","Archivo","Notas","CreadoPor"];const r=csvOrders.map(o=>{const mg=o.maq_cost&&o.maq_price?pct(parseFloat(o.maq_cost),parseFloat(o.maq_price)):"";return[o.id,fDT(o.created_at),o.order_type,o.priority,o.production_number,o.agent||"",o.client,o.client_company,o.client_phone?(o.client_lada||"+52")+" "+o.client_phone:"",o.client_email||"",o.client_rfc||"",o.product,o.product_type,o.quantity,o.paper_type,o.paper_grammage||"",o.width_cm,o.height_cm,o.colors,o.ink_front||"",o.ink_back||"",o.finishes,o.estimated_hours,o.price,o.maq_cost,o.maq_price,mg,o.maq_provider||o.maquila_provider,o.maquila_phone||"",o.maquila_email||"",SM[o.stage]?.l,o.due_date,(o.waste_log||[]).reduce((s,w)=>s+(w.pliegos||0),0),(o.waste_log||[]).reduce((s,w)=>s+(w.qty||0),0),(o.machine_log||[]).reduce((s,e)=>s+(e.minutes||0),0),o.proof_approved?fDT(o.proof_approved):"",o.file_name||"",o.notes,o.created_by||""]});const out="\uFEFF"+[h,...r].map(row=>row.map(c=>'"'+String(c||"").replace(/"/g,'""')+'"').join(",")).join("\n");const b=new Blob([out],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="PrintFlow_"+new Date().toISOString().slice(0,10)+".csv";a.click()}} style={bs(C.ac)}>📥 CSV</button>}
          <div style={{background:rC[user]+"12",color:rC[user],padding:"4px 10px",borderRadius:8,fontSize:10,fontWeight:600}}>{rL[user]}</div>
          <button onClick={()=>{setUser(null);setUserLogin(null)}} style={{...bs(C.sf,C.t2),border:"0.5px solid "+C.bd}}>Salir</button>
        </div>
      </div>

      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        {view==="pipeline"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Dashboard</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{orders.length} órdenes · {orders.filter(o=>!o.stage.includes("delivered")).length} activas</p>{(user==="admin"||isSec(user))&&<WeeklyReport orders={orders} role={user} chemicals={chemicals} plates={plates} maintenance={maintenance} userLogin={userLogin}/>}<MaquilaTracker orders={orders} onAction={handleAction} role={user} userLogin={userLogin}/><Pipeline orders={orders} role={user} onAction={handleAction}/></div>}
        {view==="tasks"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Mis Pendientes</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{myTasks.length} pendiente{myTasks.length!==1?"s":""}{search?" · filtro: \""+search+"\"":""}</p>
          {(()=>{const staleTasks=myTasks.filter(o=>getStale(o));const normalTasks=staleTasks.length>0?myTasks.filter(o=>!getStale(o)):myTasks;return <>{staleTasks.length>0&&<div style={{background:C.dn+"08",border:"1.5px solid "+C.dn+"25",borderRadius:14,padding:14,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:8}}>{staleTasks.slice().sort((a,b)=>hoursAgo(b.timeline?.length>0?b.timeline[b.timeline.length-1].date:b.created_at)-hoursAgo(a.timeline?.length>0?a.timeline[a.timeline.length-1].date:a.created_at)).map(o=>{const s=getStale(o);const st=SM[o.stage];return <div key={o.id} onClick={()=>handleAction(o.id,"detail")} style={{background:C.bg,borderRadius:10,padding:10,cursor:"pointer",borderLeft:"3px solid "+C.dn,boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div><div style={{fontSize:12,fontWeight:700}}>{o.client}</div><div style={{fontSize:10,color:C.t2}}>{o.product_type}{o.quantity?" · "+Number(o.quantity).toLocaleString()+" pzas":""}</div></div>
                <div style={{background:C.dn+"15",color:C.dn,padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:800}}>{s?.lb}</div>
              </div>
              <div style={{display:"flex",gap:4,marginTop:4}}><span style={{background:(st?.c||C.t3)+"15",color:st?.c,padding:"1px 6px",borderRadius:5,fontSize:9,fontWeight:600}}>{st?.l}</span>{o.due_date&&new Date(o.due_date)<new Date()&&<span style={{background:C.dn+"12",color:C.dn,padding:"1px 6px",borderRadius:5,fontSize:9,fontWeight:600}}>RETRASO</span>}</div>
            </div>})}</div>
          </div>}
          {normalTasks.filter(searchFilter).length===0&&staleTasks.length===0?<div style={{textAlign:"center",padding:"40px 20px"}}><div style={{fontSize:48}}>✅</div><div style={{fontSize:15,fontWeight:700,marginTop:8}}>{search?"Sin resultados":"¡Sin pendientes!"}</div><div style={{fontSize:12,color:C.t2,marginTop:4}}>{search?"Intenta con otro término":"Las órdenes aparecerán aquí cuando necesiten tu atención"}</div></div>:normalTasks.filter(searchFilter).map(o=><OCard key={o.id} o={o} role={user} onAction={handleAction} busy={actionLoading===o.id} noDragHint userLogin={userLogin}/>)}</>})()}</div>}
        {view==="form"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase",textAlign:"center"}}>{editO?"Editar Orden":"Nueva Orden"}</h2><OrderForm role={user} onSubmit={editO?update:create} editOrder={editO} onCancel={()=>{setEditO(null);setView("pipeline")}} clients={clients} orders={orders}/></div>}
        ...
        {view==="board"&&(user==="produccion"||user==="admin")&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Tablero de Producción</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Arrastra órdenes entre máquinas · ⠿ para mover</p><Kanban orders={orders} onDrop={assignMachine} onAction={handleAction} role={user} maintenance={maintenance} onMaintenance={(type,machine,record)=>setMaintModal({type,machine,record})}/><MaquilaTracker orders={orders} onAction={handleAction} role={user} userLogin={userLogin}/>{user==="admin"&&<><h3 style={{fontSize:15,fontWeight:800,margin:"20px 0 4px",textTransform:"uppercase",color:"#0891b2"}}>💿 Tablero Germán</h3><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>CTP y Procesadora</p><PreprensaBoard orders={orders} onDrop={assignMachine} onAction={handleAction} onPlateRequired={(oid,mid,o,m)=>setPlateModal({oid,mid,order:o,machine:m})} maintenance={maintenance}/></>}</div>}
        {view==="board"&&isSec(user)&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Salidas</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Tus órdenes listas para entregar</p>{(()=>{const sal=orders.filter(o=>o.stage==="salidas"&&(!o.created_by||o.created_by===userLogin));return sal.length===0?<div style={{textAlign:"center",padding:"40px 20px",color:C.t3}}><div style={{fontSize:48}}>📤</div><div style={{fontSize:15,fontWeight:700,color:C.tx,marginTop:8}}>Sin órdenes en salida</div><div style={{fontSize:12,color:C.t2,marginTop:4}}>Tus órdenes aparecerán aquí cuando Producción las envíe</div></div>:<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>{sal.sort(prioSort).map(o=><div key={o.id} onClick={()=>handleAction(o.id,"detail")} style={{background:C.bg,borderRadius:14,padding:16,cursor:"pointer",borderLeft:"4px solid #16a34a",boxShadow:"0 1px 4px rgba(0,0,0,0.06)"}}><div style={{fontSize:14,fontWeight:700}}>{o.client}{o.client_company?" · "+o.client_company:""}</div><div style={{fontSize:11,color:C.t2,marginTop:2}}>{o.product_type}{o.quantity?" · "+Number(o.quantity).toLocaleString()+" pzas":""}</div>{o.due_date&&<div style={{fontSize:10,color:new Date(o.due_date)<new Date()?C.dn:C.t3,marginTop:4}}>📅 Entrega: {fD(o.due_date)}</div>}{o.price&&<div style={{fontSize:13,fontWeight:700,color:C.ok,marginTop:4}}>{fmt(o.price)}</div>}<button onClick={e=>{e.stopPropagation();handleAction(o.id,"advance","delivered")}} style={{...bt(C.ok),marginTop:10,width:"100%",justifyContent:"center"}}>✅ Marcar Entregada</button></div>)}</div>})()}<MaquilaTracker orders={orders} onAction={handleAction} role={user} userLogin={userLogin}/></div>}
        {view==="calendar"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase"}}>Calendario de Entregas</h2><Calendar orders={orders} onChangeDate={changeDate} role={user} userLogin={userLogin}/></div>}
        {view==="orders"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 14px",textTransform:"uppercase"}}>Todas ({orders.length}){search&&<span style={{fontSize:13,fontWeight:500,color:C.t2,textTransform:"none"}}> · filtro: "{search}"</span>}</h2>{orders.filter(searchFilter).sort(prioSort).map(o=><OCard key={o.id} o={o} role={user} onAction={handleAction} busy={actionLoading===o.id} noDragHint userLogin={userLogin}/>)}</div>}
        {view==="archive"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>🗂️ Archivo de Completadas</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>Órdenes entregadas organizadas por fecha</p><Archive orders={orders} role={user} onAction={handleAction} userLogin={userLogin}/></div>}
      ...
      {devolverModal&&<DevolverModal onConfirm={async(reason)=>{try{const o=orders.find(x=>x.id===devolverModal);await doAdv(devolverModal,"design");await db.addComment(devolverModal,"↩️ Devuelto a Diseño: "+reason,user);await db.notify("preprensa",devolverModal,"order_edit","↩️ Orden devuelta a Diseño — "+(o?.client||"")+" · "+(o?.product_type||"")+": "+reason,null,user);if(user==="admin")await db.addNotification("produccion",devolverModal,"order_edit","↩️ Orden devuelta a Diseño — "+(o?.client||"")+" · "+(o?.product_type||"")+": "+reason,null,user)}catch(e){showToast("❌ Error al devolver","error");reload()}setDevolverModal(null)}} onClose={()=>setDevolverModal(null)}/>}
      {plateModal&&<PlateModal order={plateModal.order} machine={plateModal.machine} onConfirm={async(size,qty)=>{try{await db.addPlate(plateModal.oid,size,qty,user);await assignMachine(plateModal.oid,plateModal.mid);await db.addComment(plateModal.oid,"📋 Placas: "+qty+" "+size+"s registradas","sistema")}catch(e){showToast("❌ Error al registrar placas","error");reload()}setPlateModal(null)}} onClose={()=>setPlateModal(null)}/>}
      {maintModal?.type==="start"&&<StartMaintenanceModal machine={maintModal.machine} onConfirm={async(notes)=>{try{await db.startMaintenance(maintModal.machine.id,notes,user);const m=await db.loadMaintenance();setMaintenance(m);const msg="🔧 "+maintModal.machine.name+" en mantenimiento"+(notes?" — "+notes:"");if(user!=="admin")await db.addNotification("admin",null,"new_order",msg,null,user);if(user!=="produccion")await db.addNotification("produccion",null,"new_order",msg,null,user)}catch(e){showToast("❌ Error al iniciar mantenimiento","error")}setMaintModal(null)}} onClose={()=>setMaintModal(null)}/>}
      {maintModal?.type==="end"&&<EndMaintenanceModal machine={maintModal.machine} record={maintModal.record} onConfirm={async(cost)=>{try{await db.endMaintenance(maintModal.record.id,cost,user);const m=await db.loadMaintenance();setMaintenance(m);await db.notify("produccion",null,"new_order","✅ "+maintModal.machine.name+" reparada — Costo: $"+parseFloat(cost).toLocaleString("es-MX",{minimumFractionDigits:2}),null,user)}catch(e){showToast("❌ Error al cerrar mantenimiento","error")}setMaintModal(null)}} onClose={()=>setMaintModal(null)}/>}
      {confirmModal&&<ConfirmModal {...confirmModal} onClose={()=>setConfirmModal(null)}/>}
      {printModal&&<PrintOrder order={printModal} role={user} onClose={()=>setPrintModal(null)}/>}
      {clientHistory&&<ClientHistory clientName={clientHistory} orders={orders} role={user} userLogin={userLogin} onClose={()=>setClientHistory(null)}/>}
      {flowDiagram&&<FlowDiagram currentStage={flowDiagram.stage} orderType={flowDiagram.type} onClose={()=>setFlowDiagram(null)}/>}
      {detailModalId&&orders.find(x=>x.id===detailModalId)&&<DetailModal order={orders.find(x=>x.id===detailModalId)} role={user} userLogin={userLogin} onClose={()=>setDetailModalId(null)} onPrint={o=>setPrintModal(o)}/>}
