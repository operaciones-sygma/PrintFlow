  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        {view==="pipeline"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Dashboard</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{orders.length} órdenes · {orders.filter(o=>!o.stage.includes("delivered")).length} activas</p>{(user==="admin"||user==="secretaria")&&<WeeklyReport orders={orders} role={user} chemicals={chemicals} plates={plates} maintenance={maintenance}/>}<MaquilaTracker orders={orders} onAction={handleAction} role={user}/><Pipeline orders={orders} role={user} onAction={handleAction}/></div>}
        {view==="tasks"&&<div><h2 style={{fontSize:18,fontWeight:800,margin:"0 0 4px",textTransform:"uppercase"}}>Mis Pendientes</h2><p style={{fontSize:11,color:C.t2,margin:"0 0 14px"}}>{myTasks.length} pendiente{myTasks.length!==1?"s":""}{search?" · filtro: \""+search+"\"":""}</p>{myTasks.filter(searchFilter).length===0?<div style={{textAlign:"center",padding:"40px 20px"}}><div style={{fontSize:48}}>✅</div><div style={{fontSize:15,fontWeight:700,marginTop:8}}>{search?"Sin resultados":"¡Sin pendientes!"}</div><div style={{fontSize:12,color:C.t2,marginTop:4}}>{search?"Intenta con otro término":"Las órdenes aparecerán aquí cuando necesiten tu atención"}</div></div>:myTasks.filter(searchFilter).map(o=><OCard key={o.id} o={o} role={user} onAction={handleAction} busy={actionLoading===o.id} noDragHint/>)}</div>}
