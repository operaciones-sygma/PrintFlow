...
export default function PrintFlow() {
  ...
  const handleAction=useCallback((id,action,payload)=>{
    ...
    if(action==="validate_pre"){setActionLoading(id);const o=orders.find(x=>x.id===id);(async()=>{try{setOrders(p=>p.map(x=>{if(x.id!==id)return x;const u={...x,validated_by_preprensa:true,timeline:addTL(x,"✅ Validada por Pre-prensa")};if(x.validated_by_production){u.stage="design";u.timeline=addTL(u,"📝 → 🎨 Ambos validaron → Diseño",{to:"design"})}return u}));const {error:vpreErr}=await supabase.from("orders").update({validated_by_preprensa:true,...(o?.validated_by_production?{stage:"design"}:{})}).eq("id",id);if(vpreErr)throw new Error(vpreErr.message);await db.addTimeline(id,"✅ Validada por Pre-prensa",user,"#ec4899");if(o?.validated_by_production)await db.addTimeline(id,"📝 → 🎨 Ambos validaron → Diseño",user,"#ec4899");await db.notifySecs(id,"validation","✅ Pre-prensa validó la orden de "+(o?.client||"")+" — "+(o?.product_type||""),null,user,o?.created_by);showToast("✅ Orden validada por Pre-prensa")}catch(e){console.error("[validate_pre] Error:",e);showToast("❌ No se pudo validar: "+(e?.message||"error desconocido"),"error");reload()}finally{setActionLoading(null)}})()}
