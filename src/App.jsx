...
@media print{body{padding:10px 14px}@page{margin:8mm}}
    ...
    h+=`<div class="header">
      ...
      <div class="header-folio"><div class="lbl">Folio</div><div class="num">${o.production_number||o.id}</div>${o.cart_folio?'<div class="cart">🛒 '+o.cart_folio+'</div>':''}<div class="date">${pDate.getDate()} ${months[pDate.getMonth()].slice(0,3)} ${pDate.getFullYear()}</div></div>
    </div>`;
    ...
    <div style={{background:C.bg,borderRadius:20,padding:24,maxWidth:520,width:"92%",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
        <div>
          ...
          <div style={{fontSize:o.cart_folio?10:9,color:C.t3,marginTop:o.cart_folio?2:0}}>{o.id}</div>
...
function OCard({o,role,onAction,compact,busy,noDragHint,userLogin}) {
  ...
  return <div draggable={isDraggable} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    <div style={{display:"flex",gap:10}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        {o.cart_folio&&!compact&&<div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span style={{fontSize:16,fontWeight:800,color:"#06b6d4",letterSpacing:0.5,lineHeight:1}}>🛒 {o.cart_folio}</span></div>}
        <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3,flexWrap:"wrap"}}>
          <span style={{fontSize:o.cart_folio?9:10,color:C.t3}}>{o.id}</span>
                    ...
                    {wOpen&&<div style={{paddingLeft:8,marginTop:4,display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:6}}>
                      {wOrders.map(o=>{
                        ...
                        return <div key={o.id} onClick={()=>onAction(o.id,"detail")} style={{background:isCancelled?C.dn+"04":C.bg,borderRadius:10,padding:10,cursor:"pointer",borderLeft:"3px solid "+(st?.c||C.ok),boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                          {o.cart_folio&&<div style={{fontSize:13,fontWeight:800,color:"#06b6d4",letterSpacing:0.3,lineHeight:1,marginBottom:3}}>🛒 {o.cart_folio}</div>}
    ...
    {rejected.length>0&&<div>
      ...
      {showRejected&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:10}}>
        {rejected.map(o=><div key={o.id} onClick={()=>onDetail(o.id)} style={{background:C.bg,borderRadius:12,padding:12,cursor:"pointer",borderLeft:"3px solid "+C.dn,opacity:0.7,boxShadow:"0 0 0 0.5px "+C.bd}}>
          {o.cart_folio&&<div style={{fontSize:13,fontWeight:800,color:"#06b6d4",letterSpacing:0.3,lineHeight:1,marginBottom:4}}>🛒 {o.cart_folio}</div>}
...
export default function PrintFlow() {
  ...
  useEffect(() => {
    ...
    orders.forEach(o => {
      ...
      const msg = "🌐 Nuevo pedido web: " + (o.client || "Sin nombre") + " — " + (o.product_type || "") + (o.price ? " · " + new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN"}).format(o.price) : "");
  ...
  const create=useCallback(async f=>{
    ...
    const newOrder={...f,id:gid(),stage:isMaq?"maq_created":"draft",priority:f.priority||"normal",created_at:new Date().toISOString(),created_by:userLogin||user,source:"internal",validated_by_production:false,validated_by_preprensa:false,price:f.price?parseFloat(f.price):null,quantity:f.quantity?parseInt(f.quantity,10):null,estimated_hours:f.estimated_hours?parseFloat(f.estimated_hours):null,maq_cost:f.maq_cost?parseFloat(f.maq_cost):null,maq_price:f.maq_price?parseFloat(f.maq_price):null,machine_log:[],waste_log:[],comments:[],notes_log:[],current_machine:null,proof_approved:null,timeline:[{action:"📋 Orden creada",date:new Date().toISOString(),by:user,color:C.ac}]};
  ...
  const webApprove=useCallback(async id=>{
    ...
    setOrders(p=>p.map(x=>x.id!==id?x:{...x,stage:"draft",created_by:newCreator,timeline:addTL(x,"🌐 → 📝 Pedido web aprobado",{to:"draft"})}));
    try{
      const {error}=await supabase.from("orders").update({stage:"draft",created_by:newCreator}).eq("id",id);
      if(error)throw new Error(error.message);
      await db.addTimeline(id,"🌐 → 📝 Pedido web aprobado",user,C.t3);
      await db.addComment(id,"🌐 Pedido web aprobado — entra al flujo de validación",user);
      const msg="📋 Nueva orden (🌐 Web) de "+(o.client||"")+" — "+(o.product_type||"")+" requiere validación";
      ...
      showToast("✅ Pedido web aprobado → validación");
    }catch(e){showToast("❌ Error al aprobar pedido","error");reload()}
  ...
  const webReject=useCallback(async(id,reason)=>{
    ...
    }catch(e){showToast("❌ Error al rechazar","error");reload()}
    finally{setActionLoading(null);setWebRejectModal(null)}
  },[orders,user,showToast,reload]);
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{borderBottom:"0.5px solid "+C.bd,padding:"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          ...
          {(user==="admin"||isSec(user))&&<button onClick={()=>{const csvOrders=viewOrders;const h=["ID","Fecha","Tipo","Prioridad","#Prod","Agente","Cliente","Empresa","Tel","Email","RFC","Producto","TipoProd","Cant","Papel","Gramaje","Ancho","Alto","Tintas","TintasFrente","TintasVuelta","Acabados","Hrs","Precio","CostoMaq","PrecioMaq","Margen","Proveedor","ProvTel","ProvEmail","Etapa","Entrega","PlMerma","PzMerma","MinMaq","Prueba","Archivo","Notas","CreadoPor","Source","WebRef","CartFolio"];const r=csvOrders.map(o=>{const mg=o.maq_cost&&o.maq_price?pct(parseFloat(o.maq_cost),parseFloat(o.maq_price)):"";return[o.id,fDT(o.created_at),o.order_type,o.priority,o.production_number,o.agent||"",o.client,o.client_company,o.client_phone?(o.client_lada||"+52")+" "+o.client_phone:"",o.client_email||"",o.client_rfc||"",o.product,o.product_type,o.quantity,o.paper_type,o.paper_grammage||"",o.width_cm,o.height_cm,o.colors,o.ink_front||"",o.ink_back||"",o.finishes,o.estimated_hours,o.price,o.maq_cost,o.maq_price,mg,o.maq_provider||o.maquila_provider,o.maquila_phone||"",o.maquila_email||"",SM[o.stage]?.l,o.due_date,(o.waste_log||[]).reduce((s,w)=>s+(w.pliegos||0),0),(o.waste_log||[]).reduce((s,w)=>s+(w.qty||0),0),(o.machine_log||[]).reduce((s,e)=>s+(e.minutes||0),0),o.proof_approved?fDT(o.proof_approved):"",o.file_name||"",o.notes,o.created_by||"",o.source||"internal",o.web_order_ref||"",o.cart_folio||""]});const out="\uFEFF"+[h,...r].map(row=>row.map(c=>'"'+String(c||"").replace(/"/g,'""')+'"').join(",")).join("\n");const b=new Blob([out],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="PrintFlow_"+new Date().toISOString().slice(0,10)+".csv";a.click()}} style={bs(C.ac)}>📥 CSV</button>}
