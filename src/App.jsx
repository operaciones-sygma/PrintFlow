...
function Kanban({orders,onDrop,onAction,role}) {
  ...
  return <div>
    ...
    return <div key={m.id} onDragOver={e=>{e.preventDefault();setDO(m.id)}} onDragLeave={()=>setDO(null)} onDrop={e=>drop(m.id,e)} style={{background:isD?cc[type]+"10":C.sf,borderRadius:12,padding:12,border:"2px dashed "+(isD?cc[type]:mo.length>0?cc[type]+"30":"transparent"),minHeight:80}}>
      ...
      {mo.length===0?<div style={{textAlign:"center",padding:"8px 0",color:C.ph,fontSize:10}}>{isD?"⬇ Soltar aquí":"Sin órdenes"}</div>:mo.map(o=><div key={o.id} draggable onDragStart={e=>e.dataTransfer.setData("orderId",o.id)} onClick={()=>onAction(o.id,"detail")} style={{background:C.bg,borderRadius:8,padding:8,marginBottom:4,cursor:"pointer",boxShadow:"0 0 0 0.5px rgba(0,0,0,0.06)"}}>
        ...
        <div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:4,marginTop:4}}><button onClick={()=>onAction(o.id,"advance","packaging")} style={bs(C.ok)}>📦 Empaque</button><button onClick={()=>onAction(o.id,"send_maquila")} style={{...bs("#e67e22"),padding:"4px 8px"}}>🚚</button><button onClick={()=>onAction(o.id,"waste")} style={{...bs(C.sf,C.t2),padding:"4px 8px"}}>🗑️</button></div>
      </div>)}
    </div>})}</div></div>})}
  </div>;
}
...
export default function PrintFlow() {
  ...
  const [flowDiagram,setFlowDiagram]=useState(null);const [showWelcome,setShowWelcome]=useState(false);const [detailModal,setDetailModal]=useState(null);
  ...
  const create=useCallback(async f=>{
    ...
    const newOrder={...f,id:gid(),stage:isMaq?"maq_created":"draft",priority:f.priority||"normal",created_at:new Date().toISOString(),created_by:user,validated_by_production:false,validated_by_preprensa:false,price:f.price?parseFloat(f.price):null,quantity:f.quantity?parseInt(f.quantity):null,estimated_hours:f.estimated_hours?parseFloat(f.estimated_hours):null,maq_cost:f.maq_cost?parseFloat(f.maq_cost):null,maq_price:f.maq_price?parseFloat(f.maq_price):null,machine_log:[],waste_log:[],comments:[],current_machine:null,proof_approved:null,timeline:[{action:"📋 Orden creada",date:new Date().toISOString(),by:user,color:C.ac}]};
  ...
  const handleAction=useCallback((id,action,payload)=>{
    ...
    if(action==="detail"){const o=orders.find(x=>x.id===id);if(o)setDetailModal(o)}
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      {detailModal&&<DetailModal order={detailModal} role={user} onClose={()=>setDetailModal(null)} onPrint={o=>setPrintModal(o)}/>}
    </div>
  );
}
