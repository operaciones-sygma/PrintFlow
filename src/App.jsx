...
// PRINTFLOW v10.11.0 — Sub-fase A: Mover órdenes entre OCs (admin/Lupita/Vendedor)
  ...
  return <div draggable={isDraggable} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    {!compact&&canAct&&<div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap",opacity:busy?0.5:1,pointerEvents:busy?"none":"auto",position:"relative"}}>
      ...
      <div style={{display:"flex",gap:4,marginLeft:"auto"}}>
        ...
        {inOCView&&o.purchase_order_id&&!o.stage.includes("delivered")&&!o.stage.includes("cancelled")&&!o.invoice_folio&&(role==="admin"||(isSec(role)&&secOwns))&&<button onClick={()=>onAction(o.id,"move_to_oc")} style={bs(C.sf,C.ac)} title="Mover a otra OC">↔️</button>}
    ...
    {/* Cancel + Move buttons for sec/vendedor — visible outside canAct gate too */}
    {!compact&&!canAct&&!o.stage.includes("delivered")&&!o.stage.includes("cancelled")&&!o.invoice_folio&&isSec(role)&&secOwns&&<div onClick={e=>e.stopPropagation()} style={{marginTop:6,display:"flex",justifyContent:"flex-end",gap:6}}>
      ...
      <button onClick={()=>onAction(o.id,"cancel_order")} style={bs(C.sf,C.dn)} title="Cancelar orden">❌ Cancelar</button>
    </div>}
...
function AuditoriaView({orders}){
  ...
  const {sequence,gaps,duplicates,total,latest,oldest}=useMemo(()=>{
    const map={};
    ...
    const dupes=numbers.filter(n=>map[n].length>1);
    if(numbers.length===0)return {sequence:[],gaps:[],duplicates:[],total:0,latest:null,oldest:null};
    const min=numbers[0],max=numbers[numbers.length-1];
    const seq=[];const gapList=[];
    for(let i=min;i<=max;i++){
      if(map[i]){seq.push({n:i,orders:map[i],status:map[i].length>1?"duplicate":"ok"})}
      else{seq.push({n:i,orders:[],status:"gap"});gapList.push(i)}
    }
    return {sequence:seq,gaps:gapList,duplicates:dupes,total:numbers.length,latest:max,oldest:min};
  },[folioOrders]);
  const exportCSV=()=>{
    ...
    sequence.forEach(item=>{
      ...
      else{item.orders.forEach(o=>{rows.push([o.invoice_folio,item.status==="duplicate"?"DUPLICADO":"OK",o.client||"",o.production_number||"",o.id,o.invoiced_at?fDT(o.invoiced_at):"",o.invoiced_by==="secretaria"?"Lupita":(o.invoiced_by||""),o.invoice_pre_assigned?"SI":"NO",o.cancelled_at?"SI":"NO"])})}
    });
  ...
  return <div>
    ...
    {sequence.length===0?<div style={{textAlign:"center",padding:"40px 20px",color:C.t3,background:C.bg,borderRadius:10,border:"1px solid "+C.bd}}>
      ...
      {sequence.map(item=>{
        ...
        return item.orders.map((o,i)=><div key={item.n+"-"+i} style={{padding:"10px 14px",borderBottom:"1px solid "+C.bd,background:item.status==="duplicate"?C.wn+"15":"transparent",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
    ...
    <div style={{marginTop:14,padding:"12px 14px",background:C.bg,borderRadius:10,border:"1px solid "+C.bd,borderLeft:"4px solid "+C.t3,fontSize:11,color:C.t2,lineHeight:1.5}}>
      <strong style={{color:C.tx}}>Cómo interpretar:</strong> los <strong>gaps</strong> son números faltantes en la secuencia capturada — pueden ser folios cancelados en AlphaERP o capturas omitidas en PrintFlow. Los <strong>duplicados</strong> indican que el mismo folio se asignó a varias órdenes (debería estar bloqueado por el sistema; si aparecen, es alerta). Para auditoría completa, exporta el CSV y compáralo contra el reporte de AlphaERP.
    </div>
  </div>;
}

// ─── ÓRDENES DE COMPRA (v10.10.0) ─── Lista + detalle de OCs complejas
function OrdenesCompraView({purchaseOrders, orders, role, userLogin, onAction, onReload, showToast, onCreateOC, onAddProduct}){
  ...
  if (selectedOC) {
    const canAddProduct = canCreateOC && selectedOC.status !== "cancelled" && selectedOC.status !== "completed";
    return <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",margin:"0 0 10px",flexWrap:"wrap",gap:8}}>
        ...
        {canAddProduct && <button onClick={()=>onAddProduct(selectedOC)} style={{...bt(C.ac),fontSize:12,padding:"8px 14px"}}>+ Agregar producto a esta OC</button>}
      </div>
      {ocOrders.length === 0
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"14px 16px"}}>
        ...
        {view==="oc"&&(isSec(user)||user==="admin"||user==="karla")&&<OrdenesCompraView purchaseOrders={purchaseOrders} orders={orders} role={user} userLogin={userLogin} onAction={handleAction} onReload={reload} showToast={showToast} onCreateOC={createOC} onAddProduct={addProductToOC}/>}
