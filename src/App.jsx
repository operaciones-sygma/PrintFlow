...
function Analytics({orders}) {
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:4,marginBottom:16,overflowX:"auto"}}>
      {[{id:"finance",l:"💰 Financiero"},{id:"machines",l:"🏭 Máquinas"},{id:"efficiency",l:"📊 Eficiencia"},{id:"clients",l:"👥 Clientes"}].map(t=>
...
export default function PrintFlow() {
  ...
  const duplicate=useCallback(async id=>{
    ...
    const dup={...orig,id:gid(),stage:"draft",created_at:new Date().toISOString(),created_by:user,validated_by_production:false,validated_by_preprensa:false,machine_log:[],waste_log:[],comments:[],current_machine:null,proof_approved:null,deliveredAt:null,delivered_at:null,maquila_provider:null,maquila_phone:null,maquila_email:null,timeline:[{action:"📋 Duplicada de "+orig.id,date:new Date().toISOString(),by:user,color:"#5856d6"}]};
    setOrders(p=>[dup,...p]);
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Poppins',sans-serif",color:C.tx}}>
      ...
      <div style={{borderBottom:"0.5px solid "+C.bd,padding:"8px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          {(user==="admin"||user==="secretaria")&&<button onClick={()=>{const h=["ID","Fecha","Tipo","Prioridad","#Prod","Cliente","Empresa","Tel","Email","RFC","Producto","TipoProd","Cant","Papel","Ancho","Alto","Tintas","Acabados","Hrs","Precio","CostoMaq","PrecioMaq","Margen","Proveedor","ProvTel","ProvEmail","Etapa","Entrega","PlMerma","PzMerma","MinMaq","Prueba","Notas"];const r=orders.map(o=>{const mg=o.maq_cost&&o.maq_price?pct(parseFloat(o.maq_cost),parseFloat(o.maq_price)):"";return[o.id,fDT(o.created_at),o.order_type,o.priority,o.production_number,o.client,o.client_company,o.client_phone?(o.client_lada||"+52")+" "+o.client_phone:"",o.client_email||"",o.client_rfc||"",o.product,o.product_type,o.quantity,o.paper_type,o.width_cm,o.height_cm,o.colors,o.finishes,o.estimated_hours,o.price,o.maq_cost,o.maq_price,mg,o.maq_provider||o.maquila_provider,o.maquila_phone||"",o.maquila_email||"",SM[o.stage]?.l,o.due_date,(o.waste_log||[]).reduce((s,w)=>s+(w.pliegos||0),0),(o.waste_log||[]).reduce((s,w)=>s+(w.qty||0),0),(o.machine_log||[]).reduce((s,e)=>s+(e.minutes||0),0),o.proof_approved?fDT(o.proof_approved):"",o.notes]});const out="\uFEFF"+[h,...r].map(row=>row.map(c=>'"'+String(c||"").replace(/"/g,'""')+'"').join(",")).join("\n");const b=new Blob([out],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="PrintFlow_"+new Date().toISOString().slice(0,10)+".csv";a.click()}} style={bs(C.ac)}>📥 CSV</button>}
