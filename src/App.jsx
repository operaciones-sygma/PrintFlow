...
function OCard({o,role,onAction,compact,busy,noDragHint,userLogin}) {
  ...
  const secOwns=!isSec(role)||o.created_by===userLogin;const canAct=secOwns&&(st?.who===role||(st?.who==="secretaria"&&isSec(role))||st?.who==="both"&&(role==="produccion"||role==="preprensa")||role==="admin"||(o.stage==="proof_client"&&isSec(role)));const stale=getStale(o);const hp=role==="produccion"||role==="preprensa"||role==="german";
  ...
  return <div draggable={isDraggable} onDragStart={e=>e.dataTransfer.setData("orderId",o.id)}
    ...
    {!compact&&canAct&&<div onClick={e=>e.stopPropagation()} style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap",opacity:busy?0.5:1,pointerEvents:busy?"none":"auto",position:"relative"}}>
      ...
      {o.stage==="salidas"&&(role==="admin"||(isSec(role)&&o.created_by===userLogin))&&<button onClick={()=>onAction(o.id,"advance","delivered")} style={bt(C.ok)}>✅ Marcar Entregada</button>}
    ...
    {!compact&&isSec(role)&&o.created_by===userLogin&&o.order_type!=="maquila"&&<div onClick={e=>e.stopPropagation()} style={{marginTop:6}}>
    ...
    {!compact&&isSec(role)&&o.created_by===userLogin&&o.order_type==="maquila"&&!o.stage.includes("delivered")&&<div onClick={e=>e.stopPropagation()} style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap"}}>
...
export default function PrintFlow() {
  ...
  const handleAction=useCallback((id,action,payload)=>{
    if(action==="edit"){const o=orders.find(x=>x.id===id);if(!o)return;if(isSec(user)&&o.created_by!==userLogin)return;if(isSec(user)&&o.order_type!=="maquila"&&o.validated_by_production&&o.validated_by_preprensa)return;setEditO(o);setView("form")}
